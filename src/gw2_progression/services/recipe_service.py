...
async def _fetch_recipes_for_output(item_id: int) -> list[dict]:
    ...
    if not recipe_ids:
        return []
    recipes = await _gw2_fetch(f"/v2/recipes?ids={','.join(str(i) for i in recipe_ids[:5])}")
    if not recipes:
        return []
    result = recipes if isinstance(recipes, list) else [recipes]
    for r in result:
        rid = r.get("id")
        if rid:
            _cache.set(f"recipe:output:{item_id}", result)
    return result
...
async def _expand_ingredient(
    ...
    if depth < MAX_RECIPE_DEPTH and missing > 0:
        ...
        if recipes:
            ...
            multiplier = -(-missing // output_count)  # ceiling division

            sub_ings = []
            for raw in best.get("ingredients", []):
                sub_id = raw.get("item_id")
                sub_count = raw.get("count", 1) * multiplier
                sub = await _expand_ingredient(sub_id, sub_count, owned_map, prices, depth + 1)
                sub_ings.append(sub)
...
async def calculate(
    ...
    recipes_for_output = await _fetch_recipes_for_output(target_item_id)
    ...
    root = await _expand_ingredient(target_item_id, quantity, owned_map, prices, depth=0)
