...
AUTH = "gw2_progression.services.auth_service"


def _mock_using_db(db):
    """Create an async context manager mock for using_db()."""
    ctx = AsyncMock()
...
@pytest.fixture
def mock_using(mock_db):
    with patch(f"{AUTH}.using_db", return_value=_mock_using_db(mock_db)):
        yield mock_db
